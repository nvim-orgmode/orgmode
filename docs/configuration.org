...
Set an entry from [[#org-todo-keywords][org_todo_keywords]] to use as the "starting" state for repeatable todos.

If provided value does not exist in [[#org-todo-keywords][org_todo_keywords]], first entry from that list is used.
*** =win_split_mode=
:PROPERTIES:
:CUSTOM_ID: win-split-mode
:END:
...
- Default: =true=
Possible values:
- =true= - Disable [[#org-adapt-indentation][org_adapt_indentation]] by default when [[#org-startup-indented][org_startup_indented]] is enabled.
- =false= - Do not disable [[#org-adapt-indentation][org_adapt_indentation]] by default when [[#org-startup-indented][org_startup_indented]] is enabled.
...
- Default: =true=
Possible values:
- =true= - Enable [[#org-hide-leading-stars][org_hide_leading_stars]] by default when [[#org-startup-indented][org_indent_mode]] is enabled for buffer (~vim.b.org_indent_mode = true~).
- =false= - Do not modify the value in [[#org-hide-leading-stars][org_hide_leading_stars]] by default when [[#org-startup-indented][org_indent_mode]] is enabled for buffer (~vim.b.org_indent_mode = true~).
...
[[#org-timestamp-up][org_timestamp_up]]/[[#org-timestamp-down][org_timestamp_down]]
...
*** =org_id_ts_format=
:PROPERTIES:
:CUSTOM_ID: org-id-ts-format
:END:
- Type: =string=
- Default: =%Y%m%d%H%M%S=
Format of the id generated when [[#org-id-method][org_id_method]] is set
to =ts=.
...
- =uuid= - Use [[#org-id-uuid-program][org_id_uuid_program]] to generate
  the id
- =ts= - Generate id from current timestamp using format [[#org-id-ts-format][org_id_ts_format]]
- =org= - Generate a random 12 digit number and prepend [[#org-id-prefix][org_id_prefix]]
...
- Type: =string | nil=
- Default: =nil=
Prefix added to the generated id when [[#org-id-method][org_id_method]] is set to =org=.
...
headline as property. More info on [[#org-store-link][org_store_link]]

*** =org_babel_default_header_args=
:PROPERTIES:
:CUSTOM_ID: org-babel-default-header-args
:END:
- Type: =table<string, string>=
...
:PROPERTIES:
:CUSTOM_ID: calendar-week-start-day
:END:
- Type: =number=
- Default: =1=
Available options:
...
Determine on which day the week will start in calendar modal (ex:[[#org-change-date][changing the date under cursor]])
...
:PROPERTIES:
:CUSTOM_ID: org-agenda-start-day
:END:
- Type: =string=
- Default: =nil=
...
[[#org-agenda][org_agenda]] mapping. It is possible to combine multiple
...
- =target= (=string?=) --- name of the file to which the capture content
  ...
  added to the [[#org-default-notes-file][org_default_notes_file]] file
...
:PROPERTIES:
:CUSTOM_ID: org-agenda-min-height
:END:
- Type: =number=
- Default: =16=
...
:PROPERTIES:
:CUSTOM_ID: org-priority-highest
:END:
- Type: =string|number=
- Default: =A=
...
:PROPERTIES:
:CUSTOM_ID: org-priority-default
:END:
- Type: =string|number=
- Default: =B=
...
:END:
Adjust behavior of the calendar modal (ex: [[#org-change-date][changing the date under cursor]]).

*** =calendar.round_min_with_hours=
:PROPERTIES:
:CUSTOM_ID: calendarround-min-with-hours
:END:
- Type: =boolean=
- Default: =true=
...
:PROPERTIES:
:CUSTOM_ID: calendarmin-big-step
:END:
- Type: =number=
- Default: =15=
...
:PROPERTIES:
:CUSTOM_ID: calendarmin-small-step
:END:
- Type: =number=
- Default: same as [[#org-time-stamp-rounding-minutes][org_time_stamp_rounding_minutes]]
...
[[#org-meta-return][org_meta_return]] which is a normal mode mapping. If
...
:END:
- Mapped to: =<Leader>or=
Refile current headline to a destination org-file. Same as [[#org-refile][org_refile]] but from agenda view.
...
:PROPERTIES:
:CUSTOM_ID: org-agenda-show-help
:END:
- Mapped to: =g?=
...
:PROPERTIES:
:CUSTOM_ID: org-capture-show-help
:END:
- Mapped to: =g?=
Show help popup with mappings.
...
- Minute - Example date: =<2021-10-01 Fri 10:3|0>= becomes
  =<2022-10-02 Sat 11:3|5>=. See [[#org-time-stamp-rounding-minutes][org_time_stamp_rounding_minutes]] for steps configuration.
...
Same as [[#org-timestamp-up][org_timestamp_up]], just opposite
direction.
...
TODO states prompt (see [[#org-todo-keywords][org_todo_keywords]]) if it's enabled.

**** =org_todo_prev=
:PROPERTIES:
:CUSTOM_ID: org-todo-prev
:END:
- Mapped to: =ciT=
...
are any links stored with [[#org-store-link][org_store_link]], pressing =<TAB>= to autocomplete the input will show
...
Generate a link to the closest headline. If [[#org-id-link-to-org-use-id][org_id_link_to_org_use_id]] is
=true=, it appends the =ID= property to the headline, and generates link with that id to be inserted via
[[#org-insert-link][org_insert_link]]. When [[#org-id-link-to-org-use-id][org_id_link_to_org_use_id]] is =false=,
...
=$PATH=. See [[#org-custom-exports][org_custom_exports]] if you want to add your own export options.
...
:PROPERTIES:
:CUSTOM_ID: org-edit-src-save-exit
:END:
- Mapped to: =<Leader>'=
...
:PROPERTIES:
:CUSTOM_ID: org-edit-src-show-help
:END:
- Mapped to: =g?=
...
:PROPERTIES:
:CUSTOM_ID: inner-heading-from-root
:END:
...
:PROPERTIES:
:CUSTOM_ID: around-heading-from-root
:END:
- Mapped to: =OH= (big letter =o=)
...
:PROPERTIES:
:CUSTOM_ID: inner-subtree-from-root
:END:
...
:PROPERTIES:
:CUSTOM_ID: around-subtree-from-root
:END:
- Mapped to: =OR= (big letter =o=)
...
(~example: ==> 1:05~), updating any of the dates via [[#org-timestamp-up][org_timestamp_up]]/[[#org-timestamp-down][org_timestamp_down]] automatically recalculates this value.
...
1. Setting =header-args= on multiple levels:
   ...
   1. Configuration ([[#org-babel-default-header-args][org_babel_default_header_args]])
...
Running [[#org-babel-tangle][org_babel_tangle]] will create file =~/org/todos.lua= with this content:
...
Running [[#org-babel-tangle][org_babel_tangle]] will create file =~/org/my_tangled_file.lua= with this content:
...
For adding/changing TODO keyword colors see [[#org-todo-keyword-faces][org-todo-keyword-faces]]
*** Menu
:PROPERTIES:
:CUSTOM_ID: menu
:END:
