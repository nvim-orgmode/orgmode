...
---Check if the parsed tree is stale (buffer changed since last parse)
---Useful for async callers to detect when node positions may be invalid
---@return boolean true if tree is stale and should be re-parsed
function OrgFile:is_tree_stale()
  local bufnr = self:bufnr()
  if bufnr < 0 then
    return false -- File source, content is immutable
  end
  if not self._parse_tick then
    return true -- Never parsed with tick tracking
  end
  return self._parse_tick ~= vim.api.nvim_buf_get_changedtick(bufnr)
end
