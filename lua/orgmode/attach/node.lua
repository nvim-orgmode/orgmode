...
function AttachNode:_make_absolute(path)
  ...
  local resolved = fs_utils.substitute_path(path, base)
  if not resolved then
    resolved = fs_utils.substitute_path(vim.fs.joinpath('.', path), base)
    assert(resolved)
  end
  return vim.fs.normalize(resolved, { expand_env = false })
end
...
---Toggle the `org_attach_auto_tag`.
---
...
function AttachNode:toggle_auto_tag(onoff)
  ...
  if config.org_attach_auto_tag and self.headline then
    -- `toggle_tag()` eventually ends up calling `vim.fn.bufnr()` inside
    -- `OrgFile`, which is disallowed inside `fast-api`. Thus, we schedule
    -- this change.
    vim.schedule(function()
      self.headline:toggle_tag(config.org_attach_auto_tag, onoff)
    end)
  end
end
