...
---@param fuzzy? boolean if true, use `matchfuzzy()`
---@return fun(arglead: string): string[]
function M.make_completion(root, fuzzy)
  ---@type fun(candidates:string[],basename:string[]):string[]
  local matcher
  if fuzzy then
    matcher = function(candidates, basename)
      -- Only call matchfuzzy() if it won't break.
      if basename:len() <= 256 then
        return vim.fn.matchfuzzy(candidates, basename)
      end
      return candidates
    end
  else
    matcher = function(candidates, basename)
      local pattern = '^' .. vim.pesc(basename)
      return vim.tbl_filter(function(candidate)
        return candidate:find(pattern) ~= nil
      end, candidates)
    end
  end
  ...
  return function(arglead)
    ...
    if basename ~= '' then
      candidates = matcher(candidates, basename)
    end
...
function M.select_attachment(action, attach_dir)
  return Input.open(action .. ' attachment: ', '', M.make_completion(attach_dir, true))
end
