...
function OrgFile:insert_headline(headline, after)
  ...
  if after then
    ...
    if last then
      insert_at = last.position.end_line
    else
      insert_at = 0
    end
  end
  ...
  if headline.node then
    ...
    local keyword = headline.keyword and (' ' .. headline.keyword) or ''
    lines = { stars .. keyword .. ' ' .. headline.text }
  end
  ...
  return hl or reloaded.headlines[#reloaded.headlines]
end
