...
function OrgMappings:_todo_change_state(direction)
  ...
  -- Handle repeating tasks with TodoState-aware reset
  for _, date in ipairs(repeater_dates) do
    self:_replace_date(date:apply_repeater())
  end

  -- Capture the state BEFORE reset for the note
  local new_todo = item:get_todo()

  -- Use TodoState for sequence-aware reset (ESSENTIAL for multiple sequences)
  ...
  if reset_keyword then
    ...
    -- Fallback to old reset behavior if TodoState doesn't provide a reset
