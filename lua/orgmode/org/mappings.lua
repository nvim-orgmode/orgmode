...
function OrgMappings:_handle_repeating_task(headline, old_state, new_state)
  ...
  -- Update dates based on repeaters
  ...
  -- Add the LAST_REPEAT property if logging is enabled
  if log_repeat_enabled then
    headline:set_property('LAST_REPEAT', '[' .. now:to_string() .. ']')

    -- Create the state change log entry
    ...
    -- Add the state change to the appropriate drawer
...
function OrgMappings:_todo_change_state(direction, use_fast_access)
  ...
  -- Handle becoming done: add CLOSED timestamp
  if becoming_done then
    ...
    if has_repeater then
      -- Process repeating task
      self:_handle_repeating_task(headline, old_state, new_state)

      -- Get the state to reset to (from property or config)
      local reset_todo = todo_state:get_reset_todo(headline)

      -- Reset state based on REPEAT_TO_STATE property or config
      if reset_todo then
  ...
  -- Handle becoming undone: remove CLOSED timestamp
