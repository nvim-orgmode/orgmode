...
describe('Hyperlink.at_pos', function()
  ---@param obj any sut
  ...
  local function assert_link_in_range(line, lb, rb, opt)
    for pos = lb, rb do
      local link = Hyperlink.at_pos(line, pos)
  ...
  local function assert_not_link_in_range(line, lb, rb)
    for pos = lb, rb do
      local nil_link = Hyperlink.at_pos(line, pos)
      assert(
        not nil_link,
  ...
  it('could have an empty description like [[someurl]]', function()
    ...
    local link_str = Hyperlink.at_pos(line, 1):to_str()
